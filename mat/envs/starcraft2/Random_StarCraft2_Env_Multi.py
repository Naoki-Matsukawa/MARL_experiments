...
class RandomStarCraft2EnvMulti(MultiAgentEnv):
    ...
    def reset(self):
        ...
        if self.target_n_agents > self.n_agents:
            for i in reversed(range(self.target_n_agents - self.n_agents)):
                ...
                fake_agent_state[0][-(i+1)] = 1.
                global_state = np.append(global_state, fake_agent_state, axis=0)

                fake_agent_obs = np.zeros((1, np.shape(local_obs)[-1]))
                fake_agent_obs[0][-(i+1)] = 1.
                local_obs = np.append(local_obs, fake_agent_obs, axis=0)

                fake_available_action = np.ones((1, np.shape(available_actions)[-1]))
                available_actions = np.append(available_actions, fake_available_action, axis=0)
    ...
    def step(self, actions):
        ...
        try:
            ...
            if self.target_n_agents > self.n_agents:
                for i in reversed(range(self.target_n_agents - self.n_agents)):
                    ...
                    fake_done = np.array([True])
                    dones = np.append(dones, fake_done, axis=0)
        ...
        if self.target_n_agents > self.n_agents:
            for i in reversed(range(self.target_n_agents - self.n_agents)):
                ...
                fake_reward = np.array([[reward]])
                rewards = np.append(rewards, fake_reward, axis=0)
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if unit.health > 0:
            ...
            # transfer
            avail_actions = [0] * ft.get_act_dim()
